...
class LocalFileAdapter:
    ...
    This class mimics the behavior of an uploaded file object (specifically for
    getting file name and binary content) expected by the ingestion logic.
    """

    def __init__(self, file_path: str):
        """
        Initializes the adapter with a local file path.
    ...
    def getbuffer(self) -> bytes:
        """
        Reads the file content into a bytes buffer.
...
def answer_ai_report_question(
    ...
    """
    Retrieval Augmented Generation (RAG) function to answer questions about the
    AI Engineering Report.

    This function serves as the system-under-test for LangSmith evaluation,
    taking an input question and returning an answer. It handles document
    ingestion, index building, and RAG invocation internally.

    Args:
        inputs: Dictionary containing the question, e.g., {"question": "What is RAG?"}
        data_path: Optional path to the AI Engineering Report text file. Defaults
                   to `data/The 2025 AI Engineering Report.txt` relative to the
                   project root.
        chunk_size: Size of text chunks for document splitting.
        chunk_overlap: Overlap between adjacent chunks.
        k: Number of documents (chunks) to retrieve from the index.

    Returns:
        Dictionary with the answer, e.g., {"answer": "RAG stands for..."}.
        Returns an error message in the 'answer' field on failure.
    """
...
def correctness_evaluator(run: Run, example: Example) -> dict:
    ...
    This function uses a Gemini model to compare the RAG system's output against
    a golden reference answer.

    Args:
        run: The LangSmith Run object containing the actual outputs
        example: The LangSmith Example object containing the expected outputs and input

    Returns:
        A dictionary compliant with LangSmith custom evaluator output,
        including 'key', 'score' (1 for correct, 0 for incorrect), 'reasoning',
        and 'comment'.
    """
...
def run_evaluation(
    ...
    This function configures and executes the LangSmith `evaluate` call
    with the specified dataset, RAG function, and set of evaluators.

    Args:
        dataset_name: Name of the dataset in LangSmith to fetch evaluation examples from.
                      (default: "AgenticAIReportGoldens")
        evaluator_type: Type of evaluator(s) to use. Must be one of
                        'correctness' (custom LLM-as-a-Judge), 'cot_qa' (LangChain's),
                        or 'all'. (default: "correctness")
        experiment_prefix: Optional custom prefix for the LangSmith experiment name.
        description: Optional custom description for the LangSmith experiment.
        chunk_size: Chunk size parameter passed to the RAG function. (default: 1000)
        chunk_overlap: Chunk overlap parameter passed to the RAG function. (default: 200)
        k: Number of documents to retrieve (k) parameter passed to the RAG function. (default: 5)
    """
    print(f"\n{'='*80}")
...
def main():
    """
    Parses command-line arguments and initiates the LangSmith evaluation.

    This function sets up the CLI with options for dataset name, evaluator type,
    experiment metadata, and RAG configuration parameters (chunk_size, k).
    It also checks for required environment variables (`LANGSMITH_API_KEY`
    and `GOOGLE_API_KEY`) before running the evaluation.
    """
